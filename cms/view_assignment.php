...
<section class="summary mt-5">
	<div class="container">
		<div class="row">
			<div class="col-12">
				<h2 class="text-center mb-4">Assignment Marks of <?php echo $batch; ?> Batch</h2>
				...
				<?php if ($getFromU->teacher_only() === true && $_SESSION['teacher_code'] == $teacher_code) : ?>
					<a href="assignment.php?course_id=<?php echo $course_id ?>&batch=<?php echo $batch; ?>" class="btn btn-sm btn-primary mb-3">Add New Assignment</a>
				<?php endif ?>
				  ...
				  <thead style="background-color: #B7CDDA;">
				    <tr class="text-center">
				      ...
				      <th scope="col">Student Name</th>
							...
							<?php
								$columns = $getFromU->view_assignment_column_by_batch_course_id($batch, $course_id);
								foreach ($columns as $column) {
									$assignment_no = $column->Field;
							?>
							<th scope="col"><?php echo ucwords($assignment_no); ?></th>
							<?php } ?>
						...
						?>
					      ...
					      <td><?php echo $student_name; ?></td>
								...
								<?php
									...
									foreach ($columns as $column) {
										$assignment_no = $column->Field;
										$ass_values = $getFromU->view_assignment_by_roll2($batch, $course_id , $roll);
										//var_dump($att_date);


										foreach ($ass_values as $ass_value) {
											$value = $ass_value->{"$assignment_no"};
