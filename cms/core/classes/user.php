<?php
    ...
    public function admin_and_teacher_only()
    {
  	...
  	{
  		$sql = "SELECT * FROM routine ORDER BY ex_date";
  		$stmt = $this->pdo->prepare($sql);
  		$stmt->execute();
  	...
  	{
  		$sql = "SELECT * FROM result ORDER BY stu_id";
  		$stmt = $this->pdo->prepare($sql);
  		$stmt->execute();
  	...
  	public function viewBatches()
  	{
		...
		public function viewassfedback()
		{
			$sql = "SELECT * FROM ass_feedback";
			$stmt = $this->pdo->prepare($sql);
			$stmt->execute();
		...
		public function viewassignById($ass_id)
		{
			$sql = "SELECT * FROM assignment WHERE ass_ID = :ass_id";
			$stmt = $this->pdo->prepare($sql);
			$stmt->bindValue(":ass_ID", $ass_id);
			$stmt->execute();
		...
		public function viewResultByID($r_id)
		{
    ...
    public function teacher_update($teacher_code, $teacher_name, $t_position, $t_speciality, $file)
    {
		...
		public function result_update($stu_id, $course_id, $mark, $grade)
    {


        $sql = "UPDATE `result` SET `stu_id` = :stu_id, `course_id` = :course_id, `mark` = :mark,   `grade` = :grade  WHERE `r_id` = :r_id ";
        $stmt = $this->pdo->prepare($sql);
    ...
    public function delete($table, $course_id)
    {
		...
		public function ex_delete($table, $ex_id)
    {
		...
		public function a_delete($table, $ass_id)
    {
    	$sql = 	"DELETE FROM $table WHERE ass_ID = :ass_id";
    	$stmt = $this->pdo->prepare($sql);
    	$stmt->bindParam(":ass_ID", $ass_id);
    	return $stmt->execute();
    }
		public function f_delete($table, $f_id)
    {
    	$sql = 	"DELETE FROM $table WHERE f_id = :$f_id";
    	$stmt = $this->pdo->prepare($sql);
